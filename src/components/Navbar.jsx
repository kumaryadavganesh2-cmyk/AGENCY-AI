...
const Navbar = ({ theme, setTheme }) => {
  ...
  return (
    ...
    backdrop-blur-md font-medium bg-white/50 dark:bg-gray-900/70"
    >
      <img
      ...
      sm:text-sm ${!sidebarOpen ? "max-sm:w-0 overflow-hidden" : "max-sm:w-60 max-sm:pl-10"} max-sm:fixed 
      top-0 bottom-0 right-0 max-sm:min-h-screen max-sm:h-full 
      max-sm:flex-col max-sm:bg-primary max-sm:text-white 
      max-sm:pt-20 flex sm:items-center gap-5 transtionall-all`}
      >
        <img
      ...
      <div className="flex item-center gap-2 sm:gap-4">
        ...
        item-center gap-2 bg-primary text-white px-6 py-2 rounded-full
